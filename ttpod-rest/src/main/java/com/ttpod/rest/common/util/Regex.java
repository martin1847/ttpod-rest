...
public abstract class Regex {
    ...
    ** 中文字符匹配 : \u4e00-\u9fa5
    */
    private static final String ZHCHAR = "\\u4e00-\\u9fa5";
    ...
    public static final Pattern VALID_MENTION_OR_LIST = Pattern.compile("([^a-z0-9" + ZHCHAR + "_!#$%&*" + AT_SIGNS_CHARS + "]|^|RT:?)(" + AT_SIGNS + "+)([a-z0-9" + ZHCHAR + "_]{1,20})(/[a-z" + ZHCHAR + "][a-z0-9" + ZHCHAR + "_\\-]{0,24})?", Pattern.CASE_INSENSITIVE);
     ...
     * 判断是否为正常的中文字符组成的昵称
     */
    public static final Pattern VALID_NICKNAME = Pattern.compile("[a-zA-Z0-9"+ ZHCHAR +"\\-_]{2,20}");

    /**
     * 判断本地注册用户密码是否正常
     */
    public static final Pattern PASSWORD = Pattern.compile("[a-zA-Z0-9]{4,20}");
     ...
     * </br>
     */
    public static final Pattern VALID_DOT_IDS = Pattern.compile("\\d+(,\\d+)*");
     ...
     * </br>
     */
    public static final Pattern VALID_UNDERLINE_IDS = Pattern.compile("\\d+(_\\d+)*");

    /**
     * 是否为正常的昵称 汉字a-z0-9-_ 4~20位
     * @param name
     * @return
     */
    public static boolean isValidNickName(String name){
        return name !=null && VALID_NICKNAME.matcher(name).matches();
    }

    /**
     * 是否为正常的密码 字母 数字  6~20 位
     * @param password  密码
     * @return
     */
    public static boolean isVaildPassword(String password){
       return password != null && PASSWORD.matcher(password.trim()).matches();
    }
     ...
     * @param ids
     */
    public static boolean isVaildIds(String ids){
        return ids != null && VALID_UNDERLINE_IDS.matcher(ids.trim()).matches();
    }
    ...
    public static void main(String[] args) {
        ...
        for(String name : tests){
            System.out.println(
                   name+"\t"+ isVaildIds(name)

            );
        }


        System.out.println("用户名："+isValidNickName("IAMROBOT3Tomi_Enn"));
        System.out.println(isVaildPassword(""));
    }
